...
va_detect_cmd_t va_command_to_id(const char *command,const char *variable_text)
{
	//printf("%s\r\n", command);
    // printf("Variable Text : %s \n",variable_text);
    if (strcmp(command, "Change_Colour") == 0)
    {
        if (strcmp(variable_text, "Red") == 0){
            return Red;
        }
        else if (strcmp(variable_text, "Blue") == 0){
            return Blue;
        }
        else if (strcmp(variable_text, "Green") == 0){
            return Green;
        }
        
    } 
    else if (strcmp(command, "SetMax") == 0)
    {
        return SetMax;
    }
